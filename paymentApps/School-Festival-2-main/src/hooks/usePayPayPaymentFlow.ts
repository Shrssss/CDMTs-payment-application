...
// 実体はuseMockPaymentFlow(カード・PayPay共通の実装)。ここではPayPay固有の値
...
export function usePayPayPaymentFlow({
  ...
  const { handlePay, submitting } = useMockPaymentFlow({
    ...
    dispatch,
    setPaymentState,
    mockOrderId: MOCK_PAYPAY_ORDER_ID,
    notImplementedMessage: "PayPay決済(PaySys)は未実装です。後任担当者が実装予定です。",
  });

  return { handlePayWithPayPay: handlePay, submitting };
}
